...
# this is the same function that is in your "SICSSL_dfm.R" script
SICSSL_dfm<-function(text,
                    ngrams=1:2,
                    stop.words=TRUE,
                    min.prop=.01){
...
SICSSL_dfm(dox, ngrams=2)

SICSSL_dfm(dox, stop.words = FALSE)

SICSSL_dfm(dox, min.prop=.25)
